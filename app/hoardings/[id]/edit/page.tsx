...
export default function EditHoarding() {
  ...
  useEffect(() => {
    const fetchHoarding = async () => {
      try {
        ...
        setError(err.response?.data?.message || err.message || "Failed to fetch hoarding");
      } finally {
        setFetching(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const widthCm = formData.width ? parseFloat(formData.width) * 30.48 : null;
      const heightCm = formData.height ? parseFloat(formData.height) * 30.48 : null;
      
      await hoardingsAPI.update(id, {
        code: formData.code,
        city: formData.city,
        area: formData.area,
      ...
      setError(err.response?.data?.message || err.message || "Failed to update hoarding");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AppLayout>
      <div>
        ...
        <h1>Edit Hoarding</h1>
        {error && (
          <div className="card" style={{ background: "var(--danger-color-light)", color: "var(--danger-color)", marginBottom: "20px" }}>
            {error}
          </div>
        )}
        <div className="card">
